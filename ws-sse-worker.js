// ✅ ACTIVE — SharedWorker runtime của SSE client. BẮT BUỘC deploy lên Static Files
// cùng thư mục cha của js/global.js (workerScriptUrl() trong global.js tự dò URL này).
// Không thể gộp inline vào global.js: SharedWorker định danh theo URL script — blob URL
// mỗi tab một khác → không chia sẻ instance → multi-connection. Vì vậy phải là file riêng.
'use strict';
...
function onTokenReceived(newToken) {
    ...
    _tokenRequested = false;
    // Parse expiry từ token body. Token 3-phần "<dbKey>|<aus_id>|<exp>" (hoặc cũ 2-phần
    // "<aus_id>|<exp>") → exp LUÔN là phần cuối. Lấy [length-1] để đúng cả hai (G2).
    try {
        ...
        var _p = decoded.split('|');
        _tokenExpiry = Number(_p[_p.length - 1]) || 0;
    } catch (_) {
