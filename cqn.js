...
const PWA_DEBOUNCE_MS  = 1500;      // gộp CQN dồn dập → 1 lần gọi procedure/user
...
async function handleRowid(sub, rowid) {
    ...
    try {
        ...
        // Notification MỚI/unread → đẩy PWA push server-side (async, cô lập lỗi).
        // KHÔNG gọi ở handleDeleteRowid (mark-read/deleted) hay handleFullScan.
        triggerPwa(ausId, sub.dbKey);
    } catch (err) {
...
// ──────────────────────────────────────────────
// PWA push server-side — chạy khi có notification MỚI (chỉ handleRowid).
// CÔ LẬP HOÀN TOÀN khỏi đường CQN/SSE: mọi lỗi nuốt tại chỗ, KHÔNG await ở
// caller → notification real-time luôn tới dù PWA lỗi.
// Debounce theo dbKey:ausId để CQN dồn dập (kể cả re-fire do UPDATE send_pwa)
// chỉ gọi procedure 1 lần/user trong cửa sổ PWA_DEBOUNCE_MS.
// ──────────────────────────────────────────────
const _pwaDebounce = new Map();   // 'dbKey:ausId' → timer

function triggerPwa(ausId, dbKey) {
    const key = dbKey + ':' + ausId;
    const prev = _pwaDebounce.get(key);
    if (prev) clearTimeout(prev);
    _pwaDebounce.set(key, setTimeout(() => {
        _pwaDebounce.delete(key);
        runPwa(ausId, dbKey).catch(() => {});   // async, KHÔNG await; runPwa đã tự nuốt lỗi
    }, PWA_DEBOUNCE_MS));
}

async function runPwa(ausId, dbKey) {
    // Package pkg_push_notification_pwa nằm ở DB APEX (vd apex_tnc), DBLINK sang schema
    // dữ liệu (tnc). Map dbKey của notif → pool chứa package qua registry.pwaDbKey.
    const entry  = registry.getDb(dbKey);
    const pwaKey = (entry && entry.pwaDbKey) ? entry.pwaDbKey : dbKey;

    if (!registry.isReady(pwaKey)) {
        console.error('[PWA] skip %s:%s — pool "%s" chưa sẵn sàng (kiểm tra db-registry.json)', dbKey, ausId, pwaKey);
        return;   // pool PWA hỏng → bỏ qua, KHÔNG ảnh hưởng notification/SSE
    }
    ...
    let errMsg = null;
    try {
        conn = await registry.getConnection(pwaKey);
        // Procedure loop mọi notif chưa gửi của user (send_pwa IN 'N','E') và tự
        // claim-commit idempotent → gọi lặp/đa nguồn an toàn. Ghi pwa_push_log per-row.
        await conn.execute(
            `BEGIN pkg_push_notification_pwa.send_notification_pwa_v2(:ausId); END;`,
            { ausId: Number(ausId) }
        );
        await conn.commit();
        ok = true;
        console.log('[PWA] ok %s:%s (via %s)', dbKey, ausId, pwaKey);
    } catch (err) {
        errMsg = err.message;
        console.error('[PWA] fail %s:%s (via %s) %s', dbKey, ausId, pwaKey, err.message);
    } finally {
        if (conn) await conn.close().catch(() => {});
    }
    // Log console phía client: SSE event phụ 'pwa_status' (KHÔNG buffer — chỉ để log).
    try {
        deliverToUser(dbKey, String(ausId), { type: 'pwa_status', ok: ok, err: errMsg || undefined });
    } catch (_) { /* SSE có thể không mở — bỏ qua */ }
}
